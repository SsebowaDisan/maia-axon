...
function subtitleForCompany(
  company: Company,
  mode: Extract<SearchMode, "google_analytics" | "google_ads">,
) {
  if (mode === "google_analytics") {
    return company.ga4_property_id
      ? `GA4 Property - ${company.ga4_property_id}`
...
>(function CompanySelector({ companies, query, mode, onQueryChange, onSelect }, ref) {
  const Icon = mode === "google_analytics" ? BarChart3 : Building2;
  const emptyLabel =
    mode === "google_analytics"
      ? "No companies with GA4 access matched your search."
      : "No companies with Google Ads access matched your search.";

  return (
    <div
      ref={ref}
      className="absolute bottom-full left-14 mb-3 w-[360px] rounded-[24px] border border-line bg-panel p-3 shadow-card"
    >
      <Input
        autoFocus
