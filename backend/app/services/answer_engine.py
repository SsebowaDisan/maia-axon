...
class AnswerResponse:
    ...
    # 3–5 follow-up questions in the user's language, generated by the
    # answer model from what the corpus actually covers. Surfaced as
    ...
    response = client.chat.completions.create(
        ...
        messages=[{
            ...
            "content": (
                ...
                f"{source_summary}\n\n"
                "Categories:\n"
                "- 'qa' = the user wants information FROM the sources. This is the DEFAULT — "
                "any well-formed question, including meta-questions about the document itself "
                "('what is this pdf about?', 'summarize this book', 'what topics does it cover?', "
                "'give me an overview'), should be 'qa'. Lookups, explanations, definitions, and "
                "*typical / recommended values* all count.\n"
                "    EN 'what is the pdf about?', 'summarize this document', 'overview please'\n"
                ...
                "    DE 'welche Werte sind üblich?', 'worum geht es in dem Dokument?'\n"
                ...
                "- 'ambiguous' = ONLY for queries that are too sparse to retrieve against — "
                "a single isolated word with no verb or context ('nozzles?', 'ok', 'hmm'), "
                "or a request to do something with no referent ('do it', 'show me'). "
                "Meta-questions about the document are NEVER ambiguous — they're 'qa'. "
                "When in doubt, return 'qa'.\n\n"
...
def qa_agent(
    ...
    base_rules = (
        ...
        "11. CROSS-BOOK SYNTHESIS: when sources come from multiple different books "
        "(distinguishable by their document_name in the source headers), every book "
        "that contributes substantively to the topic MUST appear in your citations. "
        "Compare and contrast where the books differ; reinforce where they agree. "
        "Do not let one book dominate when others discuss the same topic.\n"
        "12. EDUCATIONAL FRAMING: the user is learning from these books without "
        "reading them in full. When introducing a topic, briefly define key terms "
        "and provide enough context that a non-expert can follow. Lead with the "
        "concept, then back it with source quotes — not the other way around.\n"
        f"13. {STRUCTURED_RESPONSE_GUIDANCE}"
    )
    rules_block = (
    ...
    suggested_questions_instruction = (
        "- suggested_questions is an array of 3 to 5 follow-up questions the user "
        "could ask NEXT, written from the user's perspective. Every suggested "
        "question MUST be answerable from the provided sources (not from general "
        "knowledge); use the breadth of topics actually present in the sources to "
        "guide the user toward what these books cover well. When the user's "
        "current question was only partially answerable from the sources, prefer "
        "suggestions that pivot to adjacent topics the sources DO cover, so the "
        "user can keep learning. CRITICAL: write each suggested question in the "
        "EXACT SAME LANGUAGE as the user's question (Dutch query → Dutch "
        "suggestions, French → French, English → English, etc.). Phrase them "
        "naturally, not as headlines."
    )

    if formula_listing:
        response_format_instruction = (
            "Return valid JSON with keys answer_markdown, used_sources, "
            "unsupported_claims, and suggested_questions. "
            ...
            f"{suggested_questions_instruction}"
        )
    else:
        response_format_instruction = (
            "Return valid JSON with keys claims, unsupported_claims, and suggested_questions.\n"
    ...
    # gpt-4o for the main grounded answer. Faster + cheaper than the
    # gpt-5 reasoning path we briefly used; in practice the citation-
    # discipline of the prompt + retrieval quality matters more than
    # extra reasoning tokens here.
    response = client.chat.completions.create(
        model="gpt-4o",
        max_tokens=4096,
        response_format={"type": "json_object"},
    ...
    try:
        ...
        suggested_questions = [
            str(item).strip()
            for item in (payload.get("suggested_questions") or [])
            if str(item).strip()
        ][:5]
...
def calculation_agent(
    ...
    # values from values that need to be looked up. The main grounded-
    # answer call uses gpt-4o for speed/cost; calculations stay on gpt-5.
